...
class UniterEncoder(nn.Module):
    ...
    def iais_singular(self, txt_attn, img_attn, cross_attn, length, modal):
        # print(modal, txt_attn.shape, img_attn.shape, cross_attn.shape)
    ...
    def iais_soft(self, txt_attn, img_attn, cross_attn, modal):
        softmax = nn.Softmax(dim=-1)
        
        if modal == 'L':
            pseudo_txt_attn = cross_attn.detach() @ img_attn @ torch.transpose(cross_attn.detach(), 0, 1)
            txt_attn, pseudo_txt_attn = softmax(txt_attn), softmax(pseudo_txt_attn)
            iais_loss = self.KLDivLoss(txt_attn.log(), pseudo_txt_attn) + self.KLDivLoss(pseudo_txt_attn.log(), txt_attn)

        elif modal == 'V':
            pseudo_img_attn = cross_attn.detach() @ txt_attn @ torch.transpose(cross_attn.detach(), 0, 1)
            img_attn, pseudo_img_attn = softmax(img_attn), softmax(pseudo_img_attn)
            iais_loss = self.KLDivLoss(img_attn.log(), pseudo_img_attn) + self.KLDivLoss(pseudo_img_attn.log(), img_attn)
        ...
        for i, layer_module in enumerate(self.layer):  # every layer_module is a bert_layer
            if IAIS and i == len(self.layer) - 1:
                ...
                for j, (input_len, nbb) in enumerate(
                    ...
                    if IAIS == 'L-soft':
                        iais_loss = self.iais_soft(txt_attn[j, :input_len, :input_len], img_attn[j, :nbb, :nbb],
                                                   t2i_attn[j, :input_len, :nbb], 'L')
                    elif IAIS == 'V-soft':
                        iais_loss = self.iais_soft(txt_attn[j, :input_len, :input_len], img_attn[j, :nbb, :nbb],
                                                   i2t_attn[j, :nbb, :input_len], 'V')
                    elif IAIS == 'L-singular':
