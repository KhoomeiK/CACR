...
class ItmRankDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db, neg_sample_size=1, IAIS=False):
    ...
    def __getitem__(self, i):
        ...
        # id_pairs is a list containing 3 items, first is positive sample, second is visual negative sample, third is linguistic negative sample
    ...
    def _collect_inputs(self, id_pairs):
        ...
        for txt_id, img_id in id_pairs:
            ...
            # mask
            if self.IAIS:
                txt_attn_masks_full = torch.ones(len(input_ids), dtype=torch.long)
                txt_attn_masks = torch.ones(len(input_ids), dtype=torch.long)
                txt_attn_masks[0] = 0  # mask [CLS]
                txt_attn_masks[-1] = 0  # mask [SEP]
                img_attn_masks = torch.ones(num_bb, dtype=torch.long)
                inputs.append((input_ids, img_feat, img_pos_feat, txt_attn_masks_full, txt_attn_masks, img_attn_masks))
            else:
                attn_masks = torch.ones(len(input_ids) + num_bb, dtype=torch.long)
...
def itm_rank_collate(inputs, IAIS=False):
    if IAIS:
        (input_ids, img_feats, img_pos_feats, txt_attn_masks_full, txt_attn_masks, img_attn_masks
         ) = map(list, unzip(concat(i for i in inputs)))
    else:
        (input_ids, img_feats, img_pos_feats, attn_masks,
         ) = map(list, unzip(concat(i for i in inputs)))
    # txt_lens = [i.size(0) for i in input_ids]
    input_ids = pad_sequence(input_ids, batch_first=True, padding_value=0)  # [sample_num, max_tl]
    ...
    sample_size = len(inputs[0])  # 3
    assert all(sample_size == len(i) for i in inputs)

    # bs, max_tl = input_ids.size()  # batch_size(bs) = sample_size * opts.batch_size
    # out_size = attn_masks.size(1)  # 84
    # gather_index = get_gather_index(txt_lens, num_bbs, bs, max_tl, out_size)  # [sample_num, 84]
    if IAIS:
        ...
        img_attn_masks_tmp = pad_sequence(img_attn_masks, batch_first=True, padding_value=0)  # [sample_num, max_nbb]
...
class ItmRankDatasetHardNegFromText(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        input_ids = self.txt_db.combine_inputs(input_ids)  # add ids of [cls] and [sep] to input_ids
        input_ids = input_ids.unsqueeze(0)
        position_ids = torch.arange(0, input_ids.size(1), dtype=torch.long).unsqueeze(0)

        neg_img_ids = sample_negative(self.img_name_list, [gt_img_fname],
                                      self.neg_sample_size)
        img_ids = [gt_img_fname] + neg_img_ids
        # process image features (gt always first)
        img_feats, img_pos_feats, num_bbs = map(list, unzip(map(self._get_img_feat, img_ids)))
        img_feat = pad_tensors(img_feats, num_bbs)
        img_pos_feat = pad_tensors(img_pos_feats, num_bbs)

        if self.IAIS:
            txt_attn_masks_full = [torch.ones(len(input_ids[0]), dtype=torch.long) for _ in range(len(img_ids))]
            ...
            txt_attn_masks_full = pad_sequence(txt_attn_masks_full, batch_first=True,
                                               padding_value=0)  # [sample_num, max_tl]
            txt_attn_masks_tmp = txt_attn_masks
            img_attn_masks_tmp = pad_sequence(img_attn_masks, batch_first=True,
                                              padding_value=0)  # [sample_num, max_nbb]
            # all the masks below is [sample_num, max_tl+max_nbb]
            attn_masks = torch.cat([txt_attn_masks_full, img_attn_masks_tmp], dim=1)
            txt_attn_masks = torch.cat([txt_attn_masks_tmp, torch.zeros_like(img_attn_masks_tmp[0])]).unsqueeze(0)
            img_attn_masks = torch.cat([torch.zeros_like(txt_attn_masks_tmp), img_attn_masks_tmp[0]]).unsqueeze(0)
            batch = {'input_ids': input_ids,
                     'position_ids': position_ids,
                     'img_feat': img_feat,
                     'img_pos_feat': img_pos_feat,
            ...
            out_size = attn_masks.size(1)
            gather_index = get_gather_index([tl] * len(img_ids), num_bbs, len(img_ids), tl, out_size)

            batch = {'input_ids': input_ids,  # [1, tl]
                     'position_ids': position_ids,  # [1, tl]
                     'img_feat': img_feat,
                     'img_pos_feat': img_pos_feat,
                     'attn_masks': attn_masks,
                     'gather_index': gather_index}
    ...
    def __getitem__(self, i):
        ...
        for txt_id in txt_ids:
            ...
            if self.IAIS:
                ...
                txt_attn_masks[0] = 0  # mask [CLS]
                txt_attn_masks[-1] = 0  # mask [SEP]
                all_txt_attn_masks_full.append(txt_attn_masks_full)
                all_txt_attn_masks.append(txt_attn_masks)
        input_ids = pad_sequence(all_inputs, batch_first=True, padding_value=0)
        position_ids = torch.arange(0, input_ids.size(1), dtype=torch.long).unsqueeze(0)

        if self.IAIS:
            txt_attn_masks_full = pad_sequence(all_txt_attn_masks_full, batch_first=True,
                                               padding_value=0)  # [sample_num, max_tl]
            txt_attn_masks_tmp = pad_sequence(all_txt_attn_masks, batch_first=True,
                                              padding_value=0)  # [sample_num, max_tl]
            img_attn_masks = [torch.ones(nbb, dtype=torch.long) for _ in range(len(txt_ids))]
            img_attn_masks_tmp = pad_sequence(img_attn_masks, batch_first=True,
                                              padding_value=0)  # [sample_num, max_nbb]
            # all the masks below is [sample_num, max_tl+max_nbb]
            attn_masks = torch.cat([txt_attn_masks_full, img_attn_masks_tmp], dim=1)
            txt_attn_masks = torch.cat([txt_attn_masks_tmp[0], torch.zeros_like(img_attn_masks_tmp[0])]).unsqueeze(0)
            img_attn_masks = torch.cat([torch.zeros_like(txt_attn_masks_tmp[0]), img_attn_masks_tmp[0]]).unsqueeze(0)
            batch = {'input_ids': input_ids,
                     'position_ids': position_ids,
                     'img_feat': img_feat,
                     'img_pos_feat': img_pos_feat,
            ...
            for i, tl in enumerate(txt_lens):
                attn_masks.data[i, :tl + nbb].fill_(1)
            out_size = attn_masks.size(1)
            gather_index = get_gather_index(txt_lens, [nbb] * len(txt_ids), len(txt_ids), tl, out_size)
...
class ItmValDataset(DetectFeatTxtTokDataset):
    ...
    def __init__(self, db_dir, img_dir, mini_batch_size=400, IAIS=False):
        super().__init__(db_dir, img_dir)
        del self.lens
        self.txt2img = self.txt_db.txt2img  # every txt corr.to 1 img
        self.img2txts = self.txt_db.img2txts  # every img corr.to 5 txts
    ...
    def get_batch(self, i, img_ids):
        ...
        input_ids = example['input_ids']  # list of token_ids
        input_ids = self.txt_db.combine_inputs(input_ids)  # tensor with special token
        input_ids = input_ids.unsqueeze(0).expand(len(img_ids), -1).clone()  # [len(img_ids), tl]
        position_ids = torch.arange(0, input_ids.size(1), dtype=torch.long).unsqueeze(0)
...
class ItmEvalDataset(ItmValDataset):
    ...
    def __getitem__(self, i):
        '''
        i: idx of txt.
        mini_batches contain the i-th txt and all imgs.
        '''
        mini_batches = []
